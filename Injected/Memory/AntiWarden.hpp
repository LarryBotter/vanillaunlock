...
//	opcode is an int and not a short!
int __fastcall SMSG_WARDEN_DATA_HandlerDetour(int a1, int opcode, int a3, int pDataStore)
{
	if (opcode == SMSG_WARDEN_DATA)
	{
		if (*(int*)0x00CE8978 != 0)
		{
			int vtable = *(int*)(*(int*)0x00CE897C);
			...
			if ((wardenModule & 0xFF) == 0 && wardenModuleAddress != wardenModule)
			{
				...
				*(byte*)(wardenModule + 0x309F) = 0xE9;
				...
				//*(byte*)(wardenModule + 0x12AD) = 0;	------------- DANGER !!! --------------
	...
	det->Restore();
	int res = ((int(__fastcall*)(int, int, int, int))det->target)(a1, opcode, a3, pDataStore);
	det->Apply();
	return res;
}
	...
	for (int i = 0; i != len; ++i)	
		if (old_bytes_map.find(to_compare + i) == old_bytes_map.end()) 
			*(byte*)(buffer + i) = *(byte*)(to_compare + i);		
			...
			//bypass MEM_CHECK for detoured functions and patches
